...
grep -h '^JNIEXPORT .* Java_' /home/patrik/Downloads/arm/classpath-0.99/include/*.h | \
...
find /home/patrik/Downloads/arm/classpath-0.99/native/jni -name \*.c | \
	xargs grep -h '^Java_' | \
...
find /home/patrik/Downloads/arm/classpath-0.99/native/jni -name \*.cpp | \
	xargs grep -h '^Java_' | \
        LC_ALL=C sed -e 's,^\(Java_[a-z_A-Z0-9]*\).*$,\1,' >> $TMPFILE2
find /home/patrik/Downloads/arm/classpath-0.99/native/jni -name \*.cpp | \
