...
//	public static final char pathSeparatorChar = pathSeparator.charAt(0);
	...
	static {
		...
		// Default is Unix
		if ((separator = System.getProperty("file.separator")) == null) {
			separator = "/";
		}
		separatorChar = separator.charAt(0);
		if ((pathSeparator = System.getProperty("path.separator")) == null) {
			pathSeparator = ":";
		}
	...
	public static final boolean isRootDirectory(String path) {
		...
		if (!isWindows)
			return false;
		if (path.length() > 2 && path.length() <= 3 && isLetter(path.charAt(0)))
			return true;
		return false;
	}
	...
	public static final String toCanonicalForm(String path) {
		...
		SimpleTokenizer st = new SimpleTokenizer(tmppath, separator);
		...
		if (st.hasMoreTokens())
			do {
				...
				if (s.equals("..")) {
					...
					int idx = tmpstr.lastIndexOf(separatorChar);
