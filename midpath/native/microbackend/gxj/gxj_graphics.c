 ...
 */
//gxj_screen_buffer* gxj_get_image_screen_buffer_impl(const gxj_screen_buffer *img,
...
//        sbuf->pixelData = (gxj_pixel_type *) img->pixelData;
//        sbuf->alphaData = (img->alphaData != NULL) ? (gxj_alpha_type *) img->alphaData : NULL;
//    } else {
...
gx_fill_triangle(jint color, const jshort *clip,
		  const gxj_screen_buffer *dst, int dotted,
                  int x1, int y1, int x2, int y2, int x3, int y3) {
  gxj_screen_buffer *sbuf = (gxj_screen_buffer *)getScreenBuffer(dst);
...
gx_copy_area(const jshort *clip,
	      const gxj_screen_buffer *dst, int x_src, int y_src,
...
static unsigned short alphaComposition(jint src, unsigned short dst, unsigned char As) {
    unsigned char Rs = (unsigned char) (src >> 16);
    unsigned char Rd = (unsigned char) ((((dst & 0xF800) << 5) | (dst & 0xE000)) >> 13);
    int pRr = ((int) Rs - Rd) * As + Rd * 0xff;
    unsigned char Rr = (unsigned char) (div(pRr));

    unsigned char Gs = (unsigned char) (src >> 8);
    unsigned char Gd = (unsigned char) (((dst & 0x07E0) >> 3) | ((dst & 0x0600) >> 9));
    int pGr = ((int) Gs - Gd) * As + Gd * 0xff;
    unsigned char Gr = (unsigned char) (div(pGr));

    unsigned char Bs = (unsigned char) (src);
    unsigned char Bd = (unsigned char) ((dst & 0x001F) << 3) | ((dst & 0x001C) >> 2);
    int pBr = ((int) Bs - Bd) * As + Bd * 0xff;
    unsigned char Br = (unsigned char) (div(pBr));
...
gx_draw_rgb(const jshort *clip,
        const gxj_screen_buffer* screen_buffer, jint *rgbData,
             jint offset, jint scanlen, jint x, jint y,
             jint width, jint height, jboolean processAlpha) {
    int diff;

    gxj_screen_buffer* sbuf = (gxj_screen_buffer*) getScreenBuffer(screen_buffer);
 ...
 */
void gx_draw_line(jint color, const jshort *clip, gxj_screen_buffer *sbuf, int dotted, int x1, int y1, int x2, int y2) {
