...
class DisplayEventListener implements EventListener {
	 ...
	 */
	DisplayEventListener(EventQueue theEventQueue, DisplayContainer theDisplayContainer) {
		...
		eventQueue.registerEventListener(EventTypes.FOREGROUND_NOTIFY_EVENT, this);
	}
	...
	public void process(Event event) {
		 ...
		 */
		DisplayAccess da = displayContainer.findDisplayById(nativeEvent.intParam4);
		...
		if (da != null) {
			...
			case EventTypes.KEY_EVENT:
				...
				if (Logging.REPORT_LEVEL < Constants.LOG_DISABLED
					...
					dc.handleKeyEvent(nativeEvent.intParam1, nativeEvent.intParam2);
				}
				return;

			case EventTypes.PEN_EVENT:
				dc.handlePointerEvent(nativeEvent.intParam1, nativeEvent.intParam2, nativeEvent.intParam3);
				return;
			...
			case EventTypes.PEER_CHANGED_EVENT:
				dc.handlePeerStateChangeEvent(nativeEvent.intParam1, nativeEvent.intParam2, nativeEvent.intParam3);
			...
			case EventTypes.FOREGROUND_NOTIFY_EVENT:
				...
				dc.handleDisplayForegroundNotifyEvent((nativeEvent.intParam1 != 0));
				return;
