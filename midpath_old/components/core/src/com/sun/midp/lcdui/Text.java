...
public class Text {
    ...
    /**
     * When a paint occurs use the cursor index to know when to paint the
     * cursor.
     */
    public static final int PAINT_USE_CURSOR_INDEX = 0;

    /**
     * When a paint occurs try to find the best value for the cursor index based
     * on the x,y coordinates of the cursor.
     */
    ...
    /**
     * Sets up a new inout structure used in various text methods. IMPL_NOTE
     * javadoc
     * 
     * @param font
     *            the font to use
     ...
     * @param options
     *            any of NORMAL | INVERT | HYPERLINK | TRUNCATE
     * @param offset
     *            the first line pixel offset
    ...
    /**
     * Paints the text in a single line, scrolling left or right as necessary to
     * keep the cursor visible within the available width for the text. The
     * offset of the text after the paintLine call, whether modified or not, is
     * returned.
     * <p>
     * If the cursor is null, signifying an uneditable TextField is being
     * painted, the text will not be scrolled left or right, and the returned
     * value will always equal the <code>offset</code> argument passed in to
     * this method.
     * 
     * @param g
     ...
     *            the available height for the text
     * @param cursor
     *            TextCursor object to use for cursor placement
     * @param offset
     *            the pixel offset of the text (possibly negative)
     * @return the current scroll offset
     */
    public static int paintLine(Graphics g, String str, Font font, int fgColor, int w, int h, TextCursor cursor, int offset) {
        ...
        //
        if (cursor != null && cursor.option == PAINT_USE_CURSOR_INDEX && cursor.index >= 0 && cursor.index <= str.length()) {
            int pos = offset;
            if (cursor.index > 0) {
     ...
     * TextInfo struct is updated when <code>str</code> changes, or when
     * scrolling happens. This method does not do any painting, but updates
     * <code>info</code> to be current for use by the paint routine,
     * <code>paintText</code>...
     * 
     * @param str
     *            the text to use
     * @param font
     *            the font to use for sizing
     * @param w
     *            the available width for the text
     * @param h
     *            the available height for the text
     ...
     *            the pixel offset of the text (possibly negative)
     * @param options
     *            only TRUNCATE matters here
     * @param cursor
     *            text cursor object for cursor position
     * @param info
     *            TextInfo structure to fill
     * @return true if successful, false if there was an error
     */
    // IMPL_NOTE: break into 3 simpler update methods: text, Y scroll, X scroll
    public static boolean updateTextInfo(String str, Font font, int w, int h, int offset, int options, TextCursor cursor, TextInfo info) {
        if (w <= 0 || (cursor == null && (str == null || str.length() == 0))) {
            return false;
        }
        ...
        if (info.scrollY) {
            ...
            //
            if (cursor != null && cursor.option == PAINT_GET_CURSOR_INDEX && cursor.x >= 0) {
                // cursor.y == height) {
                ...
                for (int i = lineStart; i < lineEnd; i++) {
                    ...
                    char ch = text[i];
                    if (Math.abs(curX - cursor.preferredX) < Math.abs(bestX - cursor.preferredX)) {
                        bestIndex = i;
                        bestX = curX;
                    }
        ...
        if (info.scrollX || info.isModified) {
            if (cursor != null && cursor.option == PAINT_USE_CURSOR_INDEX) {
                if (cursor.index >= info.lineStart[info.cursorLine] && cursor.index <= info.lineEnd[info.cursorLine]) {
                    ...
                    for (int i = 0; i < info.numLines; i++) {
                        ...
                        if (cursor.index >= info.lineStart[i] && cursor.index <= info.lineEnd[i]) {
                            info.cursorLine = i;
                            break;
                        }
                    }
        ...
        if (cursor != null && (info.isModified || info.scrollX || info.scrollY)) {
            if (info.numLines > info.visLines) {
                if (info.cursorLine > info.topVis + info.visLines - 1) {
                    int diff = info.cursorLine - (info.topVis + info.visLines - 1);
                    info.topVis += diff;
     ...
     *            the text to paint
     * @param font
     *            the font to use in painting the text
     * @param fgColor
     *            foreground color
     * @param fgHColor
     *            foreground hilight color
     * @param w
     *            the available width for the text
     * @param h
     *            the available height for the text
     ...
     *            text cursor object
     */
    public static void paintText(TextInfo info, Graphics g, String str, Font font, int fgColor, int fgHColor, int w, int h, int offset,
        ...
        while (currentLine < (info.topVis + info.visLines)) {
            ...
            // TODO: Remove the if block added to protect against bad info.visLines
            if (info.lineStart[currentLine] < text.length) {
                g.drawChars(text, info.lineStart[currentLine], info.lineEnd[currentLine] - info.lineStart[currentLine], offset, y,
                    Graphics.BOTTOM | Graphics.LEFT);
            }

            // draw the vertical cursor indicator if needed
            // update the cursor.x and cursor.y info
            if (cursor != null && cursor.option == PAINT_USE_CURSOR_INDEX && cursor.index >= info.lineStart[currentLine]
                    && cursor.index <= info.lineEnd[currentLine]) {

                int off = offset;
                if (cursor.index > info.lineStart[currentLine]) {
                    off += font.charsWidth(text, info.lineStart[currentLine], cursor.index - info.lineStart[currentLine]);
                }
     ...
     * 
     * @param g
     *            the Graphics to use to paint with
     * @param str
     *            the text to paint
     * @param font
     *            the font to use to paint the text
     * @param fgColor
     *            foreground color
     * @param fgHColor
     ...
     *            any of NORMAL | INVERT | HYPERLINK | TRUNCATE
     * @param cursor
     *            text cursor object to use to draw vertical bar
     * @return the width of the last line painted
     */
    public static int paint(Graphics g, String str, Font font, int fgColor, int fgHColor, int w, int h, int offset, int options,
            TextCursor cursor) {
        ...
        do {
            ...
            if (lineEnd > lineStart) {
                ...
                // the best array index to put the cursor
                //
                if (cursor != null && cursor.option == PAINT_GET_CURSOR_INDEX && cursor.x >= 0 && cursor.y == height) {
                    ...
                    for (int i = lineStart; i < lineEnd; i++) {
                        ...
                        g.drawChar(ch, curX, curY, Graphics.BOTTOM | Graphics.LEFT);
                    ...
                    //
                    if (truncate) {
                        g.drawChars(ellipsis, 0, 3, curX, curY, Graphics.BOTTOM | Graphics.LEFT);
                    }
                    ...
                    g.drawChars(text, lineStart, lineEnd - lineStart, offset, height, Graphics.BOTTOM | Graphics.LEFT);

                    //
                    // draw the ellipsis
                    //
                    if (truncate) {
                        g.drawChars(ellipsis, 0, 3, offset + font.charsWidth(text, lineStart, lineEnd), height, Graphics.BOTTOM
                                | Graphics.LEFT);
                    }
                }
            }
            ...
            //
            if (cursor != null && cursor.option == PAINT_USE_CURSOR_INDEX && cursor.index >= lineStart && cursor.index <= lineEnd) {

                int off = offset;
                if (cursor.index > lineStart) {
                    off += font.charsWidth(text, lineStart, cursor.index - lineStart);
                }
     ...
     * 
     * @param g
     *            the graphics to use to draw the image
     * @param x
     *            the x location of the image
     * @param y
     *            the y location of the image
     * @param w
     *            the width of the hyperlink image
     */
    public static void drawHyperLink(Graphics g, int x, int y, int w) {
        ...
        for (int j = x + w - linkWidth, first = x - linkWidth; j > first; j -= linkWidth) {
            g.drawImage(StringItemSkin.IMAGE_LINK, j, y, Graphics.BOTTOM | Graphics.LEFT);
        }
    ...
    /**
     * Gets the height in pixels and the width of the widest line in pixels for
     * the given string, calculated based on the availableWidth. size[WIDTH] and
     * size[HEIGHT] should be set by this method.
     * 
     * @param size
     *            The array that holds Item content size and location in Item
     *            internal bounds coordinate system.
     ...
     * @param offset
     *            the pixel offset for the first line
     * 
     */
    public static void getSizeForWidth(int[] size, int availableWidth, String str, Font font, int offset) {
     ...
     *            the font to use to render the string
     * @param w
     *            the available width for the string
     * @param offset
     *            the pixel offset for the first line
    ...
    /**
     * Utility method to retrieve the length of the longest line of the text
     * given the width. this may not necessarily be the entire string if there
     * are line breaks or word wraps.
     * 
     * @param str
     *            the String to use.
     * @param offset
     *            a pixel offset for the first line
     * @param width
     *            the available width for the text
     * @param font
     ...
     */
    // IMPL_NOTE - could remove and use getSizeForWidth()
    public static int getWidestLineWidth(String str, int offset, int width, Font font) {
    ...
    /**
     * Calculates the starting and ending points for a new line of text given
     * the font and input parameters. Beware of the multiple returns statements
     * within the body.
     * 
     * @param text
     *            text to process. this must not be null
     * @param font
     *            font to use for width information
     * @param inout
     *            an array of input parameters corresponding to the GNL_
     *            constants
    ...
    private static boolean getNextLine(char[] text, Font font, int[] inout) {
        ...
        while (curLoc < text.length) {
            ...
            if (((inout[GNL_OPTIONS] & TRUNCATE) == TRUNCATE) && ((inout[GNL_NUM_LINES] + 1) * inout[GNL_FONT_HEIGHT] > inout[GNL_HEIGHT])
                    && (inout[GNL_OFFSET] + curLineWidth + inout[GNL_ELLIP_WIDTH] > inout[GNL_WIDTH])) {

                leftWidth = font.charsWidth(text, curLoc + 1, text.length - curLoc - 1);
                //
                ...
                // put an ellipsis
                //
                if (inout[GNL_OFFSET] + curLineWidth + leftWidth > inout[GNL_WIDTH]) {
     ...
     * fit given the strings, fonts and maximum width in which those strings
     * should fit. Returned width is either the passed in width or a smaller
     * one. The offset in pixels for the first string is 0, second string is
     * laid out right after the first one with padding in between equal to the
     * passed in value.
     * 
     * The width in which both strings would fit given the maximum is returned
     * in size[WIDTH]. The height in which both strings would fit is returned in
     * size[HEIGHT];
     * 
     * @param size
     *            The array that returns contents size
     * @param firstStr
     ...
     *            the available width for the text
     * @param firstFont
     *            the font to render the first string in
     * @param secondFont
     *            the font to render the second string in
     * @param pad
     *            the horizontal padding that should be used between strings
     */
    public static void getTwoStringsSize(int[] size, String firstStr, String secondStr, Font firstFont, Font secondFont, int width, int pad) {
        if (((firstStr == null || firstStr.length() == 0) && (secondStr == null || secondStr.length() == 0)) || (width <= 0)) {
