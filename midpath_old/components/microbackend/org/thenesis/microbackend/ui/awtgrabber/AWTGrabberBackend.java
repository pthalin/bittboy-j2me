...
public class AWTGrabberBackend extends AWTBackend {
    ...
    public AWTGrabberBackend(int w, int h) {
        super(w, h);
        pngEncoder = new PngEncoder(screenImage, PngEncoder.NO_ALPHA, 0, 9);
    }
    ...
    public void updateARGBPixels(int[] argbPixels, int x, int y, int width, int heigth) {
        ...
        try {
            FileOutputStream outfile = new FileOutputStream(counterStringBuffer + ".png");
            byte[] pngbytes = pngEncoder.pngEncode();
            if (pngbytes == null) {
                System.out.println("[WARNING] AWTGrabberBackend: Null image");
            } else {
