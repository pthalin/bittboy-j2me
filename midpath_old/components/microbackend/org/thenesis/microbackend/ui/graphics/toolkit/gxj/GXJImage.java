 ...
 * use of the {@link #createImage(GXJImage) createImage} method. It is possible
...
public class GXJImage implements VirtualImage {
    /**
     ...
     */
    public static GXJImage createImage(int width, int height) {
        if (width <= 0 || height <= 0) {
            throw new IllegalArgumentException();
        }
        
        // SYNC NOTE: Not accessing any shared data, no locking necessary
        return new GXJImage(ImageDataFactory.getImageDataFactory().
     ...
     */
    public static GXJImage createImage(GXJImage source) {
        // SYNC NOTE: Not accessing any shared data, no locking necessary
        if (source.isMutable()) {
            return new GXJImage(ImageDataFactory.getImageDataFactory().
     ...
     */
    public static GXJImage createImage(byte[] imageData, int imageOffset,
                                    int imageLength) {
     ...
     *
     */
    public static GXJImage createImage(GXJImage image,
        ...
        if (x == 0 && y == 0 
            ...
            return 
              new GXJImage(ImageDataFactory.getImageDataFactory().
     ...
     *
     */
    public static GXJImage createImage(InputStream stream) throws java.io.IOException {
        ...
        try {
            return new GXJImage(ImageDataFactory.getImageDataFactory().createImmutableImageData(stream));
     ...
     *
     */
    public static GXJImage createRGBImage(int rgb[], int width, int height, boolean processAlpha) {
        ...
        return new GXJImage(ImageDataFactory.getImageDataFactory().createImmutableImageData(rgb, width, height, processAlpha));
    }
     ...
     * <LI>the stroke style is {@link GXJGraphics#SOLID SOLID}; and
     * </LI>
    ...
    public VirtualGraphics getGraphics() {
        if (isMutable()) {
            ...
            // a new object
            return GXJGraphics.getImageGraphics(this);
        } else {
            // SYNC NOTE: Not accessing any shared data, no locking necessary
    ...
    public void getRGB(int[] rgbData, int offset, int scanlength, int x, int y, int width, int height) {
        ...
        if (scanlength >= 0 && scanlength < width) {
            ...
            throw new IllegalArgumentException();
        } else if ((y < 0) || (x < 0) || (x + width > img_width) || (y + height > img_height)) {
            throw new IllegalArgumentException();
            ...
            if (offset < 0 || offset + ((height - 1) * scanlength) + width > buflen || offset + ((height - 1) * scanlength) < 0) {
                throw new ArrayIndexOutOfBoundsException();
            }

            imageData.getRGB(rgbData, offset, scanlength, x, y, width, height);
        }
    }
     ...
     *                  create new Image
     */
    private GXJImage(ImageData imageData) {
    ...
    public boolean render(VirtualGraphics g, int x, int y, int anchor) {
        // TODO Do nothing in this toolkit 
        return false;
    }
