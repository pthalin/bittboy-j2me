...
public class JPEGDecoder {
    ...
    private static final int MAX_HUFFMAN_SUBTREE = 256;   // max size = MAX_HUFFMAN_SUBTREE * 256
    ...
    class FrameHeader {
        ...
        public int get(InputStream in, int sof) throws IOException {
    ...
    class ScanHeader {
        ...
        // from [0] to [Ns-1]
        int get(InputStream in) throws IOException {
            //get data from file stream in
            //return 0 : correct       otherwise : error
    ...
    class QuantizationTable {
        ...
        int get(InputStream in) throws IOException {
            //get dataStream in) throws IOException{
    ...
    private int readNumber(InputStream in) throws IOException {
        int Ld;
        Ld = get16(in);
        if (Ld != 4) {
    ...
    private String readComment(InputStream in) throws IOException {
        int Lc, count = 0, i;
    ...
    private int readApp(InputStream in) throws IOException {
        int Lp;
        int count = 0;
        Lp = get16(in);
        count += 2;
    ...
    private final int get8(InputStream in) throws IOException {
        try {
            return in.read();
        } catch (IOException e) {
    ...
    //get  16-bit data
    private final int get16(InputStream in) throws IOException {
        int temp;
        try {
            temp = in.read();
            temp <<= 8;
...
*/
    ...
    private void Build_HuffTab(int tab[], int L[], int V[][]) throws IOException {
...
*/
    ...
    private int HuffmanValue(int table[], int temp[], int index[], InputStream in) throws IOException {
...
// return signed integer or 0x00FF??00 if it sees a MARKER
    ...
    private int getn(InputStream in, int n, int temp[], int index[]) throws IOException {
...
*/
    ...
    private void level_shift(int du[], int P) throws IOException {
        int i;
        if (P == 8) {
            for (i = 0; i < 64; i++)
                du[i] += 128;
...
*/
    ...
    private int decode_MCU(InputStream in, int PrevDC[],
                           int temp[], int index[]) throws IOException {
...
// input: IDCT_Source
    ...
    public void decode(InputStream in, PixelArray out) throws IOException {
