...
copy_imageregion(gxj_screen_buffer* src, gxj_screen_buffer* dest, const jshort *clip,
    ...
    if (dest == src || transform != 0) {
         ...
         */
        newSrc.pixelData =
            (gxj_pixel_type *)GXJ_malloc(width * height * sizeof (gxj_pixel_type));
        ...
        if (src->alphaData != NULL) {
            newSrc.alphaData =
                (gxj_alpha_type *)GXJ_malloc(width * height * sizeof (gxj_alpha_type));
            if (newSrc.alphaData == NULL) {
                GXJ_free(newSrc.pixelData);
 ...
 */
void gx_render_image(gxj_screen_buffer * srcBuffer,
		      gxj_screen_buffer * dstBuffer,
		      const jshort * clip,
		      jint x, jint y) {
  draw_image(srcBuffer, dstBuffer, clip, x, y);
}
 ...
 */
extern void gx_render_imageregion(gxj_screen_buffer * srcImageDataPtr,
				  gxj_screen_buffer * dstMutableImageDataPtr,
				  const jshort * clip,
				  jint x_src, jint y_src,
				  jint width, jint height,
				   jint x_dest, jint y_dest,
    ...
    gxj_screen_buffer * pdstSBuf = getScreenBuffer(dstMutableImageDataPtr);

    draw_imageregion(srcImageDataPtr, pdstSBuf,
		     clip,
		     x_dest, y_dest,
		     width, height,
		     x_src, y_src,
